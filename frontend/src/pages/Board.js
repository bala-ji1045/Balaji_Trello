...
const getBoardStyle = (board) => {
    if (board.image || board.image_url)
        ...
        return {
            // Use a consistent Trello-like purple gradient (as requested)
            backgroundColor: "#5b3b8a",
            backgroundImage:
                "linear-gradient(180deg, #5b3b8a 0%, #7b458f 48%, #a2538d 100%)",
            backgroundRepeat: "no-repeat",
            backgroundSize: "cover",
            backgroundPosition: "center center",
        };
...
const Board = (props) => {
    ...
    useEffect(() => {
        try {
            if (id) window.localStorage.setItem("lastBoardId", String(id));
        } catch (e) {
            // ignore
        }
    }, [id]);
    ...
    const desiredTitle = props.embedded
        ? document.title
        : board
        ? `${board.title} | Trello`
        : "";
    ...
    const cardIdFromQuery = useMemo(() => {
        const raw = new URLSearchParams(props.location.search).get("card");
        const idInt = parseInt(raw || "", 10);
        return Number.isFinite(idInt) ? idInt : null;
    }, [props.location.search]);

    const deepLink = useMemo(() => {
        if (!board || !cardIdFromQuery) return null;
        for (const list of board.lists || []) {
            const card = (list.items || []).find((i) => i.id === cardIdFromQuery);
            if (card) return { list, card };
        }
        return null;
    }, [board, cardIdFromQuery]);

    const closeDeepLink = () => {
        if (typeof props.onCloseDeepLink === "function") {
            props.onCloseDeepLink();
            return;
        }
        props.history.replace(`/b/${id}`);
    };
