...
class NotificationItemTargetSerializer(serializers.ModelSerializer):
    labels = LabelSerializer(many=True, read_only=True)
    board_id = serializers.IntegerField(source='list.board.id', read_only=True)
    board_title = serializers.CharField(source='list.board.title', read_only=True)
    list_id = serializers.IntegerField(source='list.id', read_only=True)
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    def get_target(self, obj):
        ...
        # Notifications about cards should include board/list identifiers.
        if obj.target._meta.object_name == 'Item':
            return NotificationItemTargetSerializer(obj.target).data
