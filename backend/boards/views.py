...
from .serializers import (AttachmentSerializer, BoardSerializer,
                          CommentSerializer, DueItemSerializer, ItemSerializer,
                          LabelSerializer, ListSerializer,
                          NotificationSerializer, ShortBoardSerializer)


_RECENT_BOARDS_CACHE_TIMEOUT_SECONDS = 60 * 60 * 24 * 7  # 7 days
...
def _get_recent_board_ids_cache(redis_key):
...
def _push_recent_board_id_cache(redis_key, board_id):
    board_id = str(board_id)
    existing = _get_recent_board_ids_cache(redis_key)
    updated = [board_id] + [bid for bid in existing if bid != board_id]
    cache.set(redis_key, updated[:20], timeout=_RECENT_BOARDS_CACHE_TIMEOUT_SECONDS)
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if sort == "recent":
            ...
            try:
                ...
                board_ids = _get_recent_board_ids_cache(redis_key)[:4]
...
class BoardDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        try:
            ...
            _push_recent_board_id_cache(redis_key, board_id)
...
class DueItemList(generics.ListAPIView):
    serializer_class = DueItemSerializer
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        lists = List.objects.filter(board__in=boards)
        queryset = Item.objects.filter(list__in=lists, due_date__isnull=False)
        ...
        if days is not None:
            try:
                ...
                if days_int > 365:
                    days_int = 365
                end = timezone.now() + timedelta(days=days_int)
                queryset = queryset.filter(due_date__lte=end)
            except ValueError:
                pass

        return queryset.select_related('list', 'list__board').prefetch_related('assigned_to').distinct().order_by('due_date', '-created_at')
...
class NotificationDetail(APIView):
    ...
    def patch(self, request, pk, *args, **kwargs):  # Mark one as read/unread
        notification = get_object_or_404(
            Notification, pk=pk, recipient=request.user)

        unread = request.data.get('unread', None)
        if unread is None:
            return Response(status=status.HTTP_204_NO_CONTENT)

        if isinstance(unread, bool):
            unread_value = unread
        elif isinstance(unread, str):
            unread_value = unread.lower() == 'true'
        else:
            unread_value = bool(unread)

        notification.unread = unread_value
